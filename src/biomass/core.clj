...
(defn get-hit
  "Given a HitID, returns a map to form an API request to the GetHIT operation."
  [hit-id]
  {:Operation "GetHIT"
   :HITId     hit-id})
...
(defn apply-signature
  "Calculates a cryptographic signature for the request by concatenating the values of the Service, Operation, and Timestamp parameters and then calculating an RFC 2104-compliant HMAC using the secret-access-key as the 'key' in base64 encoding. Result is then merged into the input map as the 'Signature' parameter."
...
(defn build-request
  "Merges a resulting hash from an mturk operation with authentication credentials, applies timestamp, calculates signature, and issues HTTP request."
